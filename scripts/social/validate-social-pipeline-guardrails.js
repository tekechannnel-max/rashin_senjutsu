...
for (const file of approvedManifestFiles) {
  ...
  for (const [index, post] of (manifest.posts || []).entries()) {
    ...
    if (!Array.isArray(designReview.miniCharacters) || designReview.miniCharacters.length !== 5) {
      addViolation('APPROVED_POST_MISSING_MINI_CHARACTER_PROOF', file, `posts[${index}] must include 5 designReview.miniCharacters entries.`);
    } else {
      designReview.miniCharacters.forEach((item, miniIndex) => {
        const rank = Number(item.rank);
        const day = Number(item.day);
        const family = Number(item.family);
        const expectedFamily = Number.isInteger(day) && day >= 1 && day <= 31 ? birthdayMiniFamilyForDay(day) : NaN;
        if (rank !== miniIndex + 1 || family !== expectedFamily || item.asset !== `birthday-family-${expectedFamily}-chibi.png`) {
          addViolation(
            'APPROVED_POST_INVALID_MINI_CHARACTER_PROOF',
            file,
            `posts[${index}].designReview.miniCharacters[${miniIndex}] must match rank/day/family asset rules.`
          );
        }
      });
    }
