...
function ensureDesignReview(post, label) {
  ...
  if (!Array.isArray(review.miniCharacters) || review.miniCharacters.length !== 5) {
    throw new Error(`${label} designReview.miniCharacters must include 5 rank/day/family proof entries.`);
  }
  for (const [index, item] of review.miniCharacters.entries()) {
    const rank = Number(item.rank);
    const day = Number(item.day);
    const family = Number(item.family);
    if (rank !== index + 1) {
      throw new Error(`${label} designReview.miniCharacters[${index}].rank must be ${index + 1}.`);
    }
    const expectedFamily = birthdayMiniFamilyForDay(day);
    if (family !== expectedFamily) {
      throw new Error(`${label} designReview.miniCharacters[${index}].family must be ${expectedFamily} for day ${day}.`);
    }
    const asset = String(item.asset || '').trim();
    if (asset !== `birthday-family-${expectedFamily}-chibi.png`) {
      throw new Error(`${label} designReview.miniCharacters[${index}].asset must match birthday-family-${expectedFamily}-chibi.png.`);
    }
  }
}
