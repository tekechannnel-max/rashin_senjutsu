...
GitHub schedule runs in UTC. The minutes are intentionally not `00` because GitHub scheduled workflows at the top of the hour can be delayed or dropped under load. The workflow checks only the morning/evening posting windows. The script refuses missed automatic posts after `SOCIAL_POST_GRACE_MINUTES` so a sleeping PC or delayed runner cannot publish the morning post hours late; duplicate protection checks existing Threads posts before publishing:

- `3,8,13,18,23,28 22,11 * * *`: checks Threads due posts at roughly `07:03/07:08/07:13/07:18/07:23/07:28` and `20:03/20:08/20:13/20:18/20:23/20:28` JST.
