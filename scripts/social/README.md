...
- `3,8,13,18,23,28 * * * *`: checks Threads every hour at `:03/:08/:13/:18/:23/:28`, but posts only inside the `07:00` and `20:00` JST grace windows.
...
- `8,23,38,53 * * * *`: checks X draft generation every hour, but writes files only inside `07:00-08:00` and `20:00-21:00` JST when `SOCIAL_X_DRAFT_GRACE_MINUTES=60`.
