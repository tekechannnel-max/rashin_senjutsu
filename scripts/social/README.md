...
Use GitHub Actions schedule as the primary automation.

Do not use local Windows Task Scheduler tasks, `powershell.exe` pop-up launches, or local daemon processes for SNS operation. Local runs are manual foreground diagnostics only, from an already-open terminal. A small Render Cron Job is an alternative only if GitHub schedules are not acceptable.
...
The primary scheduled workflow file is `.github/workflows/sns-automation.yml`.
`.github/workflows/threads-social.yml` remains available for push validation and manual Threads dispatch.
...
GitHub schedule runs in UTC. The minutes are intentionally not `00` because GitHub scheduled workflows at the top of the hour can be delayed or dropped under load. The workflow checks repeatedly, but the scripts publish or export only inside the JST due windows. The script refuses missed automatic posts after `SOCIAL_POST_GRACE_MINUTES` so a delayed runner cannot publish the morning post hours late; duplicate protection checks existing Threads posts before publishing:

- `3,8,13,18,23,28,33,38,43,48,53,58 * * * *`: checks SNS automation every hour, but posts Threads only inside the `07:00` and `20:00` JST grace windows.
...
- The same schedule checks X draft generation, but writes files only inside `07:00-08:00` and `20:00-21:00` JST when `SOCIAL_X_DRAFT_GRACE_MINUTES=60`.
