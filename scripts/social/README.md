...
Do not pin `SOCIAL_RELEASE_MODE` for normal scheduled operation. The scripts derive `prelaunch`, `prerelease`, `fix`, and `release` from the JST date; set the variable only for an explicit one-off override.
...
Use Render Cron Job `rashin-threads-scheduler` as the primary Threads automation.

Do not use local Windows Task Scheduler tasks, `powershell.exe` pop-up launches, or local daemon processes for SNS operation. Local runs are manual foreground diagnostics only, from an already-open terminal.

The Render Cron Job command must be:

```text
node scripts/social/run-scheduled-posts.js --once --only-kind=all
```

The Render Cron Job schedule must be:

```text
0,5,10,15,20,25,30 22,11 * * *
```

Render schedules use UTC, so this runs every five minutes during `07:00-07:30` and `20:00-20:30` Asia/Tokyo.

Add these Render environment variables before enabling the cron job:
...
`.github/workflows/sns-automation.yml` must not run the Threads posting job on `schedule`. `.github/workflows/threads-social.yml` remains available for push validation and manual Threads dispatch.

X draft artifacts are generated by `.github/workflows/sns-automation.yml` on schedule and by `.github/workflows/x-social-drafts.yml` for push/manual checks. These workflows have no X credentials and do not post. They export the checked manual-post draft package as a GitHub Actions artifact. A due scheduled X draft run must produce artifact files or fail; do not treat a no-file due run as successful X operation.

The scheduler refuses missed automatic Threads posts after `SOCIAL_POST_GRACE_MINUTES` so a delayed runner cannot publish the morning post hours late; duplicate protection checks existing Threads posts before publishing:

- `0,5,10,15,20,25,30 22,11 * * *`: checks Threads automation inside the `07:00` and `20:00` JST grace windows.
- `SOCIAL_POST_GRACE_MINUTES=30`: automatic posting is allowed only from `07:00` through `07:30`, and from `20:00` through `20:30`, unless manually forced.
- Outside the grace windows nothing is due.
- Inside the 07:00 JST grace window, the oracle post is due unless an existing matching Threads post is found.
- Inside the 20:00 JST grace window, the concept post is due unless an existing matching Threads post is found.
- GitHub Actions schedule separately checks X draft generation and writes files only inside `07:00-08:00` and `20:00-21:00` JST when `SOCIAL_X_DRAFT_GRACE_MINUTES=60`.
